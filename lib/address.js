...
// Z-cryptos use 2 bytes for address prefixes
const TWO_BYTE_PREFIX = true;
...
Address._classifyFromVersion = function(buffer) {
  ...
  var pubkeyhashNetwork, scripthashNetwork;
  if (TWO_BYTE_PREFIX) {
    pubkeyhashNetwork = Networks.get(buffer.readUInt16BE(0), 'pubkeyhash');
    scripthashNetwork = Networks.get(buffer.readUInt16BE(0), 'scripthash');
  } else {
    pubkeyhashNetwork = Networks.get(buffer[0], 'pubkeyhash');
    scripthashNetwork = Networks.get(buffer[0], 'scripthash');
  }
...
Address.prototype.toBuffer = function() {
  ...
  if (TWO_BYTE_PREFIX) {
    //BTCP (2 bytes)
    version = Buffer.from(this.network[this.type].toString(16), 'hex');
  } else {
    //Original - BTC (1 byte)
    version = Buffer.from([this.network[this.type]]);
  }
